import type { CategoryInfo } from '../types';

// ─── Categories ──────────────────────────────────────────────
export const CATEGORIES: CategoryInfo[] = [
  {
    id: 'backpacks',
    title: 'TRENDY',
    subtitle: 'BACKPACKS',
    slug: 'backpacks',
    image: 'https://lh3.googleusercontent.com/aida/ADBb0ujuQPAAo1qnLVdfWGCHjfcF2mTWv5IC-o-PStyWouff_BWsOBXS-HJLTa1S2M63l3LLw0-ARnnoivB9fU1vExT0fH87tg66KKoVtcq7P_4FRLjnB7fSUT9CC1JQHuH9aWcNLvUAy1DhppzaAN0WUf8whsecUq0f6IMy7FhjIgWpTXXRvdb9Jyh0FZz7sC2zJ3qbY3K760qyFDMNqIlArv8dwPbMfRCQ4_yGLuoruQcTTAQKR1fO2Lqy3cQ',
    bgColor: '#f2c94c',
    description: 'Explore our range of stylish and functional backpacks for every occasion.',
  },
  {
    id: 'college-backpacks',
    title: 'TRENDY',
    subtitle: 'COLLEGE BACKPACKS',
    slug: 'college-backpacks',
    image: 'https://lh3.googleusercontent.com/aida/ADBb0ujuQPAAo1qnLVdfWGCHjfcF2mTWv5IC-o-PStyWouff_BWsOBXS-HJLTa1S2M63l3LLw0-ARnnoivB9fU1vExT0fH87tg66KKoVtcq7P_4FRLjnB7fSUT9CC1JQHuH9aWcNLvUAy1DhppzaAN0WUf8whsecUq0f6IMy7FhjIgWpTXXRvdb9Jyh0FZz7sC2zJ3qbY3K760qyFDMNqIlArv8dwPbMfRCQ4_yGLuoruQcTTAQKR1fO2Lqy3cQ',
    bgColor: '#f2c94c',
    description: 'Backpacks designed for college life — spacious, durable, and stylish.',
    parentCategory: 'backpacks',
  },
  {
    id: 'school-backpacks',
    title: 'DURABLE',
    subtitle: 'SCHOOL BACKPACKS',
    slug: 'school-backpacks',
    image: 'https://lh3.googleusercontent.com/aida/ADBb0ujuQPAAo1qnLVdfWGCHjfcF2mTWv5IC-o-PStyWouff_BWsOBXS-HJLTa1S2M63l3LLw0-ARnnoivB9fU1vExT0fH87tg66KKoVtcq7P_4FRLjnB7fSUT9CC1JQHuH9aWcNLvUAy1DhppzaAN0WUf8whsecUq0f6IMy7FhjIgWpTXXRvdb9Jyh0FZz7sC2zJ3qbY3K760qyFDMNqIlArv8dwPbMfRCQ4_yGLuoruQcTTAQKR1fO2Lqy3cQ',
    bgColor: '#6aa5de',
    description: 'Sturdy, fun backpacks built to handle the school day.',
    parentCategory: 'backpacks',
  },
  {
    id: 'laptop-backpacks',
    title: 'PROFESSIONAL',
    subtitle: 'LAPTOP BACKPACKS',
    slug: 'laptop-backpacks',
    image: 'https://lh3.googleusercontent.com/aida/ADBb0ujuQPAAo1qnLVdfWGCHjfcF2mTWv5IC-o-PStyWouff_BWsOBXS-HJLTa1S2M63l3LLw0-ARnnoivB9fU1vExT0fH87tg66KKoVtcq7P_4FRLjnB7fSUT9CC1JQHuH9aWcNLvUAy1DhppzaAN0WUf8whsecUq0f6IMy7FhjIgWpTXXRvdb9Jyh0FZz7sC2zJ3qbY3K760qyFDMNqIlArv8dwPbMfRCQ4_yGLuoruQcTTAQKR1fO2Lqy3cQ',
    bgColor: '#a2d59b',
    description: 'Padded, protective bags engineered for your laptop and tech gear.',
    parentCategory: 'backpacks',
  },
  {
    id: 'trekking-backpacks',
    title: 'ADVENTURE',
    subtitle: 'TREKKING BACKPACKS',
    slug: 'trekking-backpacks',
    image: 'https://lh3.googleusercontent.com/aida/ADBb0ujuQPAAo1qnLVdfWGCHjfcF2mTWv5IC-o-PStyWouff_BWsOBXS-HJLTa1S2M63l3LLw0-ARnnoivB9fU1vExT0fH87tg66KKoVtcq7P_4FRLjnB7fSUT9CC1JQHuH9aWcNLvUAy1DhppzaAN0WUf8whsecUq0f6IMy7FhjIgWpTXXRvdb9Jyh0FZz7sC2zJ3qbY3K760qyFDMNqIlArv8dwPbMfRCQ4_yGLuoruQcTTAQKR1fO2Lqy3cQ',
    bgColor: '#ff7675',
    description: 'Heavy-duty packs for hikers, campers, and outdoor enthusiasts.',
    parentCategory: 'backpacks',
  },
  {
    id: 'travel',
    title: 'PREMIUM',
    subtitle: 'TRAVEL',
    slug: 'travel',
    image: 'https://lh3.googleusercontent.com/aida/ADBb0ugpMFql-z8CZ76u7Kv1ANeppDp-zkjm1-9N_DjNMJhEt86N7NrwWDE2B25Zz87CMECAjO5qkTg7HuRdXiYH6ahRxxX0h6j4sOxf4dIJBexMVtilAu-I95P-z_IhQNTQBEBNlUbLijvFtAH5lGfIGQb0rtOwTi6PWcf99-nfACCAcxYPwVRohOMk4UyxfeXBF_sQVQhM3xWcO6p34DV93BXTbrPit3VOpJPugMfx9-c9o_CY2dnztdti67I',
    bgColor: '#a2d59b',
    description: 'Travel in style with our collection of premium travel gear.',
  },
  {
    id: 'luggage',
    title: 'PREMIUM',
    subtitle: 'LUGGAGE',
    slug: 'luggage',
    image: 'https://lh3.googleusercontent.com/aida/ADBb0ugpMFql-z8CZ76u7Kv1ANeppDp-zkjm1-9N_DjNMJhEt86N7NrwWDE2B25Zz87CMECAjO5qkTg7HuRdXiYH6ahRxxX0h6j4sOxf4dIJBexMVtilAu-I95P-z_IhQNTQBEBNlUbLijvFtAH5lGfIGQb0rtOwTi6PWcf99-nfACCAcxYPwVRohOMk4UyxfeXBF_sQVQhM3xWcO6p34DV93BXTbrPit3VOpJPugMfx9-c9o_CY2dnztdti67I',
    bgColor: '#a2d59b',
    description: 'Reliable luggage for your journeys.',
    parentCategory: 'travel',
  },
  {
    id: 'duffle',
    title: 'VERSATILE',
    subtitle: 'DUFFLE',
    slug: 'duffle',
    image: 'https://lh3.googleusercontent.com/aida/ADBb0uj9WpZ0N6Y9X3z0k7n_z9a9n1y8Xp3z8Yp3v8Xp3z8Yp3z8Yp3z8Yp3z8Yp3z8Yp3z8Yp3z8Yp3z8Yp3z8Yp3z8Yp3z8Yp3z8Yp3z8Yp3z8Yp3z8Yp3z8Yp3z8Yp3z8Yp3v8Xp3z8Yp3z8Yp3z8Yp3z8Y',
    bgColor: '#f2c94c',
    description: 'Perfect for weekend getaways and gym sessions.',
    parentCategory: 'travel',
  },
  {
    id: 'accessories',
    title: 'STYLISH',
    subtitle: 'ACCESSORIES',
    slug: 'accessories',
    image: 'https://lh3.googleusercontent.com/aida/ADBb0uhmc_Pc02VGmcnle5vS1_BQTHh5B1xJhWFdlXKj65tl5yFYCJasjMQBSWmPe0p0UAlLV8k9MkEXwTj95BDJTxP6mYogzYtMWGKQhWp5DcPUWyvZ9J_Bqcxe0npjbVsw0rOWylfgCUM7iKQiNvsHBK_w2x2U890sFNfkXcx3A9qSzCsOHGAgWQwHPtje4BztfZq2SIObaKfVeKMEB0cMqVCCIU0qDWK9b3DIxdXhGPdI8korNKiFC8mAKO0',
    bgColor: '#6aa5de',
    description: 'Complete your look with our range of bags and travel accessories.',
  },
  {
    id: 'pouch',
    title: 'COMPACT',
    subtitle: 'POUCH',
    slug: 'pouch',
    image: 'https://lh3.googleusercontent.com/aida/ADBb0uhmc_Pc02VGmcnle5vS1_BQTHh5B1xJhWFdlXKj65tl5yFYCJasjMQBSWmPe0p0UAlLV8k9MkEXwTj95BDJTxP6mYogzYtMWGKQhWp5DcPUWyvZ9J_Bqcxe0npjbVsw0rOWylfgCUM7iKQiNvsHBK_w2x2U890sFNfkXcx3A9qSzCsOHGAgWQwHPtje4BztfZq2SIObaKfVeKMEB0cMqVCCIU0qDWK9b3DIxdXhGPdI8korNKiFC8mAKO0',
    bgColor: '#6aa5de',
    description: 'Small but mighty pouches for your essentials.',
    parentCategory: 'accessories',
  },
  {
    id: 'lunch-bag',
    title: 'FRESH',
    subtitle: 'LUNCH BAG',
    slug: 'lunch-bag',
    image: 'https://lh3.googleusercontent.com/aida/ADBb0uhmc_Pc02VGmcnle5vS1_BQTHh5B1xJhWFdlXKj65tl5yFYCJasjMQBSWmPe0p0UAlLV8k9MkEXwTj95BDJTxP6mYogzYtMWGKQhWp5DcPUWyvZ9J_Bqcxe0npjbVsw0rOWylfgCUM7iKQiNvsHBK_w2x2U890sFNfkXcx3A9qSzCsOHGAgWQwHPtje4BztfZq2SIObaKfVeKMEB0cMqVCCIU0qDWK9b3DIxdXhGPdI8korNKiFC8mAKO0',
    bgColor: '#a2d59b',
    description: 'Keep your meals fresh and stylish.',
    parentCategory: 'accessories',
  },
  {
    id: 'daypack',
    title: 'DAILY',
    subtitle: 'DAYPACK',
    slug: 'daypack',
    image: 'https://lh3.googleusercontent.com/aida/ADBb0ujuQPAAo1qnLVdfWGCHjfcF2mTWv5IC-o-PStyWouff_BWsOBXS-HJLTa1S2M63l3LLw0-ARnnoivB9fU1vExT0fH87tg66KKoVtcq7P_4FRLjnB7fSUT9CC1JQHuH9aWcNLvUAy1DhppzaAN0WUf8whsecUq0f6IMy7FhjIgWpTXXRvdb9Jyh0FZz7sC2zJ3qbY3K760qyFDMNqIlArv8dwPbMfRCQ4_yGLuoruQcTTAQKR1fO2Lqy3cQ',
    bgColor: '#f2c94c',
    description: 'Versatile daypacks for your daily commute.',
    parentCategory: 'accessories',
  },
  {
    id: 'tote-bag',
    title: 'CHIC',
    subtitle: 'TOTE BAG',
    slug: 'tote-bag',
    image: 'https://lh3.googleusercontent.com/aida/ADBb0uhmc_Pc02VGmcnle5vS1_BQTHh5B1xJhWFdlXKj65tl5yFYCJasjMQBSWmPe0p0UAlLV8k9MkEXwTj95BDJTxP6mYogzYtMWGKQhWp5DcPUWyvZ9J_Bqcxe0npjbVsw0rOWylfgCUM7iKQiNvsHBK_w2x2U890sFNfkXcx3A9qSzCsOHGAgWQwHPtje4BztfZq2SIObaKfVeKMEB0cMqVCCIU0qDWK9b3DIxdXhGPdI8korNKiFC8mAKO0',
    bgColor: '#ff7675',
    description: 'Spacious and stylish tote bags for any occasion.',
    parentCategory: 'accessories',
  },
  {
    id: 'junior',
    title: 'FUN & DURABLE',
    subtitle: 'PRIORITY JUNIOR',
    slug: 'junior',
    image: 'https://lh3.googleusercontent.com/aida/ADBb0ujuQPAAo1qnLVdfWGCHjfcF2mTWv5IC-o-PStyWouff_BWsOBXS-HJLTa1S2M63l3LLw0-ARnnoivB9fU1vExT0fH87tg66KKoVtcq7P_4FRLjnB7fSUT9CC1JQHuH9aWcNLvUAy1DhppzaAN0WUf8whsecUq0f6IMy7FhjIgWpTXXRvdb9Jyh0FZz7sC2zJ3qbY3K760qyFDMNqIlArv8dwPbMfRCQ4_yGLuoruQcTTAQKR1fO2Lqy3cQ',
    bgColor: '#ff7675',
    description: 'Fun, durable bags designed specifically for little explorers.',
  },
  {
    id: 'kids-trolley',
    title: 'ROLLING',
    subtitle: 'KIDS TROLLEY',
    slug: 'kids-trolley',
    image: '/junior/Trolly.jpg',
    bgColor: '#FFBB5A',
    description: 'Fun, smooth-rolling 360-degree trolley luggage designed for kids.',
    parentCategory: 'junior',
  },
  {
    id: 'trolley-backpacks',
    title: 'ROLLING',
    subtitle: 'TROLLEY BACKPACKS',
    slug: 'trolley-backpacks',
    image: '/junior/Trolly.jpg',
    bgColor: '#FFBB5A',
    description: 'Rolling backpacks and trolley luggage for school & travel.',
    parentCategory: 'junior',
  },
  {
    id: 'combo-set',
    title: 'VALUE PACKS',
    subtitle: 'COMBO SET',
    slug: 'combo-set',
    image: '/junior/Combo.jpg',
    bgColor: '#FFBB5A',
    description: 'Backpack combo value packs with tiffin pouches and matching accessories.',
    parentCategory: 'junior',
  },
  {
    id: 'premium',
    title: 'COLLECTION',
    subtitle: 'PREMIUM COLLECTION',
    slug: 'premium',
    image: 'https://lh3.googleusercontent.com/aida/ADBb0uhzm92TzHT0BVAIYYXSuCFymoj0WkZF2ABtFxBQpaNuWlYc4JQs31v9FqjXuuM9MuGjx82KACn1vS3GnpVv42-N9i6qWNMqLpFYAeAr0ZYMiru5D5N-Gh1SFh24TB5WHIvQOvKEdlnTeEYxrwa85bV0YIkcLwXW1ab9sdfFbje2EEetEQToz_5yjSeGyUVDKHcN6DQP_JmLLbHxdL6VACKP0lWfKQf6UZw_Dw_9_x84AW4dPubjbPtv-Qs',
    bgColor: '#302a5e',
    description: 'Exclusive, high-end travel gear and accessories for the discerning traveller.',
  },
  {
    id: 'premium-backpacks',
    title: 'Premium Backpacks',
    subtitle: 'Premium Backpacks',
    slug: 'premium-backpacks',
    image: '',
    bgColor: '#302a5e',
    parentCategory: 'premium',
  },
  {
    id: 'premium-luggage',
    title: 'Premium Luggage',
    subtitle: 'Premium Luggage',
    slug: 'premium-luggage',
    image: '',
    bgColor: '#302a5e',
    parentCategory: 'premium',
  },
  {
    id: 'premium-accessories',
    title: 'Premium Accessories',
    subtitle: 'Premium Accessories',
    slug: 'premium-accessories',
    image: '',
    bgColor: '#302a5e',
    parentCategory: 'premium',
  },
  {
    id: 'premium-duffle',
    title: 'Premium Duffle',
    subtitle: 'Premium Duffle',
    slug: 'premium-duffle',
    image: '',
    bgColor: '#302a5e',
    parentCategory: 'premium',
  },
];

export function getCategoryBySlug(slug: string): CategoryInfo | undefined {
  return CATEGORIES.find((c) => c.slug === slug);
}

export function getSubcategories(parentSlug: string): CategoryInfo[] {
  return CATEGORIES.filter((c) => c.parentCategory === parentSlug);
}

export function formatPrice(price: number): string {
  return new Intl.NumberFormat('en-IN', {
    style: 'currency',
    currency: 'INR',
    minimumFractionDigits: 0,
    maximumFractionDigits: 0,
  }).format(price);
}
