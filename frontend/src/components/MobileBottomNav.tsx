...
export const MobileBottomNav = ({ onSearchOpen }: { onSearchOpen: () => void }) => {
  ...
  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-[100]">
      <nav className="bg-white/95 dark:bg-[#0f1417]/95 backdrop-blur-md border-t border-gray-200/80 dark:border-white/10 pt-1.5 px-2 pb-[max(0.375rem,env(safe-area-inset-bottom))] flex items-center justify-around shadow-[0_-4px_16px_rgba(0,0,0,0.06)]">
        {navItems.map((item, index) => {
          ...
          const content = (
            <div className="flex flex-col items-center gap-0.5 relative py-0.5">
              ...
              />
              <span className={`text-[10px] font-medium tracking-tight transition-colors duration-200 ${isActive ? 'text-[#26B3FF] font-bold' : 'text-gray-400 dark:text-gray-500'}`}>
                {item.label}
              </span>
              {isActive && (
                <motion.div
