...
export const Footer = () => {
  ...
  return (
    <footer className={`${footerBg} ${isPremium ? 'text-gray-300' : '[&_*]:!text-white [&_a:hover]:!opacity-70 [&_button]:!text-white'} py-10 md:py-14 pb-28 lg:pb-24 text-sm font-outfit transition-colors duration-500`} style={footerStyle}>
        ...
        <div className="mb-8 md:mb-10 text-left">
          <Link to={homeHref} className="inline-block overflow-visible">
            <img
              src={logoSrc}
              alt="Priority Bags"
              className={`${isJunior ? 'h-8 md:h-9' : 'h-8 md:h-9 -translate-x-[19.2%]'} w-auto ${isJunior ? '' : 'brightness-0 invert'}`}
            />
          </Link>
        </div>
      ...
      {/* Bottom Bar */}
      <div className="max-w-[1720px] mx-auto px-5 md:px-10 mt-10 md:mt-16 pt-6 md:pt-8 border-t border-white/5 flex flex-col sm:flex-row items-center justify-center gap-4 md:gap-6">
        <p className="text-[9px] md:text-[10px] text-gray-500 font-semibold uppercase tracking-widest text-center">&copy; {new Date().getFullYear()} High Spirit Commercial Ventures Pvt. Ltd. All Rights Reserved.</p>
      </div>
    </footer>
  );
};
